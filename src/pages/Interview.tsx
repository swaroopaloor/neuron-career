...
const LoadingSpinner = ({ size = "default", text = "Loading..." }: { size?: "sm" | "default" | "lg", text?: string }) => {
  const sizeClasses = {
    sm: "h-4 w-4",
    default: "h-6 w-6", 
    lg: "h-8 w-8"
  ...
  <div className="min-h-screen flex items-center justify-center bg-background">
    ...
    >
      <motion.div
        ...
        className="mx-auto w-16 h-16 border-4 border-primary/20 border-t-primary rounded-full"
      />
      <motion.div
      ...
      >
        <h2 className="text-xl font-semibold">Loading Interview Suite</h2>
...
const ActionLoadingOverlay = ({ isVisible, text }: { isVisible: boolean, text: string }) => (
  <AnimatePresence>
    {isVisible && (
      <motion.div
        initial={{ opacity: 0 }}
...
export default function InterviewPage() {
  ...
  const handleStartPractice = async () => {
    ...
    try {
      ...
      setActionLoading({
        isVisible: true,
        text: "Starting Practice Interview"
      });
      ...
      setActionLoading({ isVisible: false, text: "" });
    }
  };
